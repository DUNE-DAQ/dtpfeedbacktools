...
def plotme_an_ED_v2(df_adc, df_tp, run, ntsamples, zeroped, pdf = None):
    ...
    y=df_tp['offline_ch' if is_fw_tp else 'channel'].map(lambda k: y_vals.index(k))
    ...
    if is_fw_tp:
        ...
        hits = ax2.scatter((df_tp['time_peak']-t0)/ticks_per_sample, y.values, s = 15, label = 'stitched hits', alpha =0.6)
